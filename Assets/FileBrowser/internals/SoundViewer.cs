...
public class SoundViewer : MonoBehaviour
{
	...
	IEnumerator __PreviewClip() {
		while(this.configs.aSource.clip.loadState != AudioDataLoadState.Loaded) {
			yield return null;
		}
    ...
    private IEnumerator __Start()
    {
        ...
        if (!this.configs.aSource) this.configs.aSource = GameObject.FindAnyObjectByType(typeof(AudioSource)) as AudioSource;
